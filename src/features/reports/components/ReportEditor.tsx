...
interface ReportEditorProps {
  ...
  onSaveReport: (updatedReport: DiagnosticReport) => Promise<DiagnosticReport | undefined>;
  onSubmitForReview: (report: DiagnosticReport) => Promise<DiagnosticReport | undefined>;
  onPreviewReport: (report: DiagnosticReport) => void;
  onWhatsAppShare: (report: DiagnosticReport) => void;
  onOpenBilling: (report: DiagnosticReport) => void;
  onOpenRxScanner: (report: DiagnosticReport) => void;
  externalSyncToken: number;
  onAcknowledgeCritical: (report: DiagnosticReport, notes: string) => Promise<DiagnosticReport | undefined>;
  canEdit: boolean;
...
export const ReportEditor: React.FC<ReportEditorProps> = ({
  ...
  const [criticalNotes, setCriticalNotes] = useState<string>(report.criticalResultNotes || '');
  const lastExternalSyncToken = React.useRef(externalSyncToken);
  ...
  React.useEffect(() => {
    const forceSync = externalSyncToken !== lastExternalSyncToken.current;
    if (forceSync || report.id !== currentReport.id || !isDirty) {
      setCurrentReport({ ...report });
      setIsDirty(false);
      lastExternalSyncToken.current = externalSyncToken;
    }
  }, [currentReport.id, externalSyncToken, isDirty, report]);

  const patient = currentReport.patient;
  const isFinalized = currentReport.status === 'VERIFIED' || currentReport.status === 'DISPATCHED' || currentReport.status === 'SUPERSEDED' || currentReport.status === 'ARCHIVED';
  const criticalParameters = currentReport.tests.flatMap((test) => test.parameters
    .filter((parameter) => parameter.flag === 'CRITICAL_LOW' || parameter.flag === 'CRITICAL_HIGH')
    .map((parameter) => ({ testName: test.testName, parameter })));
  const hasCriticalResults = criticalParameters.length > 0;

  const handleAcknowledgeCritical = async () => {
    const saved = await onAcknowledgeCritical(currentReport, criticalNotes);
    if (saved) {
      setCurrentReport(saved);
      setCriticalNotes(saved.criticalResultNotes || '');
      setIsDirty(false);
    }
  };
  ...
  const handleAddTestPanel = (templateId: string) => {
    ...
    const updated = {
      ...currentReport,
      tests: newTests,
      criticalResultStatus: 'PENDING' as const,
      billing: {
  ...
  const handleSave = async (): Promise<DiagnosticReport | undefined> => {
    const updated = {
      ...currentReport,
  ...
  const handleSubmitForReview = async () => {
    if (currentReport.status !== 'DRAFT') return;
    const saved = isDirty ? await handleSave() : currentReport;
    if (!saved) return;
    const submitted = await onSubmitForReview(saved);
    if (submitted) {
      setCurrentReport(submitted);
      setIsDirty(false);
    }
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-7xl mx-auto pb-16">
      ...
      <div className="bg-white p-3 sm:p-4 rounded-xl border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-3">
        ...
        <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-2">
          ...
          <button
            ...
            hidden={!canEdit}
            type="button"
            onClick={() => onOpenRxScanner(currentReport)}
          ...
          <button
            ...
            hidden={!canEdit}
            type="button"
            onClick={() => onOpenBilling(currentReport)}
            className="flex items-center justify-center gap-1.5 bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-semibold px-2.5 sm:px-3 py-2 rounded-lg border border-slate-300 transition-colors"
          >
          ...
          <button
            id="btn-submit-report-for-review"
            type="button"
            onClick={() => void handleSubmitForReview()}
            hidden={!canSubmitForReview || currentReport.status !== 'DRAFT'}
            className="col-span-2 sm:col-span-1 flex items-center justify-center gap-1.5 bg-indigo-700 hover:bg-indigo-800 text-white text-xs font-bold px-3.5 sm:px-4 py-2 rounded-lg transition-all shadow-xs"
          >
      ...
      <fieldset disabled={isFinalized || !canEdit} className="contents">
      ...
      <section className="bg-white rounded-xl border border-slate-200 shadow-xs p-4 sm:p-5">
        <div className="flex flex-wrap items-center justify-between pb-3 mb-4 border-b border-slate-200 gap-2">
          ...
          <span className="text-[11px] text-slate-500 font-mono">
            Accession: {patient.accessionNumber} | Barcode: {patient.sampleBarcode}
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-4 text-xs">
          ...
          <div>
            ...
            <input
              id="patient-accession-input"
              type="text"
              required
              value={patient.accessionNumber}
              onChange={(e) => handlePatientChange('accessionNumber', e.target.value.toUpperCase())}
              placeholder="ACC-YYYYMMDD-..."
              className="w-full px-3 py-2 border border-slate-300 rounded-lg font-mono font-bold text-teal-900 focus:ring-2 focus:ring-teal-600 focus:outline-none"
            />
          </div>
          ...
          <div>
            ...
            <select
              id="specimen-status-select"
              value={patient.specimenStatus || 'ORDERED'}
              onChange={(e) => handlePatientChange('specimenStatus', e.target.value)}
          ...
          <div>
            ...
            <input
              id="sample-collected-at-input"
              type="datetime-local"
              value={toDateTimeLocal(patient.sampleCollectedAt)}
              onChange={(e) => handlePatientChange('sampleCollectedAt', fromDateTimeLocal(e.target.value))}
          ...
          <div>
            ...
            <input
              id="sample-received-at-input"
              type="datetime-local"
              value={toDateTimeLocal(patient.sampleReceivedAt)}
              onChange={(e) => handlePatientChange('sampleReceivedAt', fromDateTimeLocal(e.target.value))}
              className="w-full px-2.5 py-2 border border-slate-300 rounded-lg font-medium text-slate-800"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              type="text"
              value={patient.sampleCollectedBy || ''}
              onChange={(e) => handlePatientChange('sampleCollectedBy', e.target.value)}
              placeholder="Collector name or initials"
          ...
          <div>
            ...
            <input
              id="sample-received-by-input"
              type="text"
              value={patient.sampleReceivedBy || ''}
              onChange={(e) => handlePatientChange('sampleReceivedBy', e.target.value)}
              placeholder="Receiving technician"
              className="w-full px-3 py-2 border border-slate-300 rounded-lg font-medium text-slate-800"
            />
          </div>
          ...
          {/* Rejection Reason */}
          {patient.specimenStatus === 'REJECTED' && (
            <div className="sm:col-span-2 lg:col-span-4">
              <label className="block text-[10px] font-bold text-rose-700 uppercase mb-1">
                Specimen Rejection Reason *
              </label>
              <textarea
                id="specimen-rejection-reason-input"
                required
                rows={2}
                value={patient.sampleRejectionReason || ''}
                onChange={(e) => handlePatientChange('sampleRejectionReason', e.target.value)}
                placeholder="Explain why this specimen was rejected and whether recollection is required."
                className="w-full px-3 py-2 border border-rose-300 rounded-lg text-slate-800 focus:ring-2 focus:ring-rose-500 focus:outline-none"
              />
            </div>
          )}
      ...
      {hasCriticalResults && (
        <section className="bg-rose-50 rounded-xl border border-rose-300 shadow-xs p-4 sm:p-5">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div>
              <h3 className="text-xs font-bold text-rose-900 uppercase tracking-wider">Critical Result Review</h3>
              <p className="mt-1 text-xs text-rose-800">These values require clinical acknowledgement before the report can be verified.</p>
              <ul className="mt-3 space-y-1 text-xs font-semibold text-rose-900">
                {criticalParameters.map(({ testName, parameter }) => (
                  <li key={`${testName}-${parameter.id}`}>
                    {testName}: {parameter.name} = {parameter.value} {parameter.unit} ({parameter.flag})
                  </li>
                ))}
              </ul>
            </div>
            <span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase ${currentReport.criticalResultStatus === 'ACKNOWLEDGED' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-200 text-rose-900'}`}>
              {currentReport.criticalResultStatus === 'ACKNOWLEDGED' ? 'Acknowledged' : 'Acknowledgement pending'}
            </span>
          </div>

          {currentReport.criticalResultStatus === 'ACKNOWLEDGED' ? (
            <p className="mt-3 text-xs font-medium text-emerald-800 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2">
              Acknowledgement recorded. You may proceed with verification.
            </p>
          ) : canAcknowledgeCritical ? (
            <div className="mt-4">
              <label className="block text-[10px] font-bold text-rose-900 uppercase mb-1">
                Clinical acknowledgement / communication note *
              </label>
              <textarea
                id="critical-result-notes-input"
                rows={2}
                required
                value={criticalNotes}
                onChange={(event) => setCriticalNotes(event.target.value)}
                placeholder="Document review, action taken, or communication with the referring clinician."
                className="w-full rounded-lg border border-rose-300 bg-white px-3 py-2 text-xs text-slate-900 outline-none focus:ring-2 focus:ring-rose-500"
              />
              <button
                id="acknowledge-critical-results-btn"
                type="button"
                disabled={!criticalNotes.trim()}
                onClick={() => void handleAcknowledgeCritical()}
                className="mt-2 rounded-lg bg-rose-700 px-3.5 py-2 text-xs font-bold text-white hover:bg-rose-800 disabled:cursor-not-allowed disabled:opacity-50"
              >
                Acknowledge critical results
              </button>
            </div>
          ) : (
            <p className="mt-3 text-xs font-semibold text-rose-800">A laboratory owner or pathologist must acknowledge these results.</p>
          )}
        </section>
      )}
    </div>
