...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin, onOfflineUnlock, offlineAvailable = false, error: externalError }) => {
  ...
  return (
    <main className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
      <form onSubmit={handleSubmit} className="w-full max-w-md bg-white rounded-2xl border border-slate-200 shadow-xl p-6 sm:p-8">
        ...
        <h2 className="text-base font-bold text-slate-900">Sign in to your laboratory</h2>
        <p className="mt-1 text-xs text-slate-500">Use the owner or staff account configured for this lab.</p>
        ...
        {offlineAvailable && onOfflineUnlock && (
          <button type="button" disabled={loading || offlineLoading} onClick={() => void handleOfflineUnlock()} className="mt-3 w-full rounded-lg border border-teal-200 bg-teal-50 px-4 py-2.5 text-sm font-bold text-teal-800 transition hover:bg-teal-100 disabled:cursor-wait disabled:opacity-60">
            {offlineLoading ? 'Unlocking cached workspaceâ€¦' : 'Unlock cached workspace offline'}
          </button>
        )}
      </form>
    </main>
